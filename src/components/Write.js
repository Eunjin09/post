...
import { createPostFB } from '../redux/modules/mgz'
...
const Write = () => {
  ...
  // 글 내용
  const [input, setInput] = useState('텍스트를 입력해주세요');
  ...
  const addPost = (e) => {
    ...
    dispatch(createPostFB({ text: input, like: "0", comment: "0", email: userEmail, layout: layout, img: file_link_ref.current?.url, day: postTime(), nick_name: userNick }))
  }
    ...
    <>
      ...
      <Form>
        <label htmlFor='file_img'>
          <input type="file" id="file_img" name='file' ref={fileInput} onChange={uploadFB} accept="image/png, image/jpeg" />
        </label>
        <h2>레이아웃 고르기</h2>
        <ul>
          <li>
        ...
        <label htmlFor="user_text">
          <p className='textt'>게시물 내용</p>
          <textarea id="user_text" placeholder="게시글 작성" onChange={(e) => {
